...
class C_VEC{
    ...
    public:
        ...
        void LEE_VEC();
        void VER_VEC();
        int RET_TAM();  // Retorno mi tama�o
        float RET_VEC1_1(int i); //Retorno 
      friend class ESTAD;  
  };
  class ESTAD{
        private:
...
int main(){
    ...
    C_VEC::C_VEC(){
     ...
     int C_VEC::RET_TAM(){return Tam;} //RETORNO DEL TAMA�O
...
//////////////////////////////////////
  ...
  float ESTAD::MEDIA(){
   Med==0.0;
    for(i=0;i<EST1.Tam;i++)Med = Med + EST1.Vec[i] ;
       Med = Med/EST1.Tam;
    return Med;   
  }
   float ESTAD::VARIANZA(){
        Var=0.0;
     for(i=0;i<EST1.Tam;i++)
           Var = Var + pow( (EST1.Vec[i] - Med),2 ) ;
           Var = Var / EST1.Tam; 
    return Var;
                }
